...
auto Trie::Put(std::string_view key, T value) const -> Trie {
  ...
  if (key.empty()) {
    std::shared_ptr<TrieNodeWithValue<T>> new_root =
        std::make_shared<TrieNodeWithValue<T>>(new_trie->root_->children_, val);
    new_trie->root_ = new_root;
    return *new_trie;
  }

  auto node_cur = new_root;  // 遍历到最后一级目录
  for (uint64_t i = 0; i < key.size() - 1; i++) {
    char key_cur = key[i];
    auto it = node_cur->children_.find(key_cur);
    if (it == node_cur->children_.end()) {
      auto new_node = std::make_shared<TrieNode>();  // 之前不存在的路径节点，需要自己创建
...
auto Trie::Remove(std::string_view key) const -> Trie {
  ...
  if (it == node_cur->children_.end()) {  // 不存在的路径节点，不需要删除
    return *new_trie;
  }
